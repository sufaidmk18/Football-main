...
def loginn(request):
    ...
    if request.method=="POST":
        ...
        if res.exists():
            ...
            if d.usertype=="manager":
                ...
                a=login.objects.get(username=username,password=password,usertype="coach")
    ...
    return render(request,'manager/login.html',{'msg':msg})
...
def mdetails(request):
    if request.method=="POST":
        ...
        manager.objects.create(login_id=request.session["lid"],name=name,dob=dob,contact=contact,housename=housename,post=post,place=place,village=village,district=district,state=state)
...
def coaches(request):
    ...
    return render(request,'manager/coach.html',context)
...
def addcoach(request):
    if request.method=="POST":
        ...
        data=login(username=username,password=password,usertype='coach').save()
...
def viewteam(request):
    return render(request,'manager/viewteam.html')

def playertable(request):
    playerr=player.objects.all()
    context={'playerr':playerr}
    return render(request,'manager/playertable.html',context)
...
def addstaff(request):
    if request.method=="POST":
        ...
        data=login(username=username,password=password,usertype='staff').save()
...
def Mngrhome(request):
    return render(request,'manager/Mngrhome.html')
